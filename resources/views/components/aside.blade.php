...
<aside id="mainSidebar" class="w-[80%] max-w-[280px] sm:w-64 bg-slate-900 text-slate-300 flex-shrink-0 flex flex-col border-r border-slate-800 fixed lg:static inset-y-0 left-0 z-50 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 shadow-2xl lg:shadow-none h-[100dvh]">
    ...
    <nav class="flex-1 px-4 space-y-1 overflow-y-auto pb-4">
        ...
        @if(Auth::user()->role === 'admin' || Auth::user()->role === 'owner')
            ...
            <a href="{{ route('admin.list-generator.index') }}"
        ...
        @if(Auth::user()->role === 'admin')
            <a href="{{ route('admin.generator-images.index') }}"
                class="flex items-center gap-3 px-3 py-2 hover:bg-slate-800 @if(request()->routeIs('admin.generator-images.*')) bg-slate-800 text-white @else hover:bg-slate-800 hover:text-white @endif rounded-xl transition-all text-sm">
                <i class="fas fa-images w-5 text-violet-400"></i> Fotos de Equipos
            </a>
